...
import { SigilData } from './yaml-loader.js';
import { generateMarkov } from './markov-generator.js';
import { parseCompleteTemplate, TemplateNode } from './template-parser.js';
  ...
  * Benefits: Better precedence, cleaner code, foundation for advanced features
  ...
  private selectFromTable(tablePath: string, exclusions: string[] = []): string {
    ...
    if (!Array.isArray(list)) {
      // Check if it's a string template instead of an array
      if (typeof list === 'string') {
        // Process string as a template (handles both quoted and unquoted YAML)
        return this.processTemplate(list);
      }
